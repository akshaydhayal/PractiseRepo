...
router.post("/",userAuth,async(req,res)=>{
    const todo=Todo({...req.body,userId:req.userId});
...
router.get("/:todoId", userAuth, async (req, res) => {
  const todo = await Todo.find({_id:req.params.todoId,userId:req.userId});
  if (todo) {
...
router.put("/:todoId",userAuth,async(req,res)=>{
    const updated=await Todo.findOneAndUpdate({_id:req.params.todoId,userId:req.userId},req.body,{
...
router.delete("/:todoId",userAuth,async(req,res)=>{
    const deleteStatus=await Todo.findOneAndDelete({_id:req.params.todoId,userId:req.userId});
